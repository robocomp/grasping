    ...
    def compute(self):
        while True:
            try:
                ...
                # get objects' poses from head camera RGB signal
                pose_image = RoboCompObjectPoseEstimation.TImage(cameraID=cam["id"], width=cam["width"], height=cam["height"], depth=3, focalx=cam["focal"], focaly=cam["focal"], alivetime=time.time(), image=image.tobytes())
                object_poses = self.objectposeestimation_proxy.getObjectPose(pose_image)
                for idx, pose in enumerate(object_poses):
                    print(f'{idx}) name : {pose.objectname}, translation : ({pose.x}, {pose.y}, {pose.z}), rotation : ({pose.rx}, {pose.ry}, {pose.rz})')
